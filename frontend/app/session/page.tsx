...
import {
  Mic, MicOff, Video, VideoOff, Phone, Settings, MessageSquare,
} from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const startSession = async () => {
      try {
        ...
        const emitReady = () => {
          console.log('[session] Emitting peer:ready — callId:', callIdRef.current);
    ...
    const onCallStart = async ({
      ...
      peerUserId: peer,
      shouldCreateOffer,
    }: { callId: string; peerUserId: string; shouldCreateOffer: boolean }) => {
      console.log('[session] call:start — shouldCreateOffer:', shouldCreateOffer);
      ...
      try {
        ...
        if (shouldCreateOffer) {
          ...
          socket.emit('webrtc:offer', { callId: cId, offer: pc.localDescription, to: peer });
        }
      } catch (err) {
    ...
    const onOffer = async ({ callId: cId, offer, from }: {
      callId: string; offer: RTCSessionDescriptionInit; from: string;
    }) => {
      ...
      try {
        ...
        socket.emit('webrtc:answer', { callId: cId, answer: pc.localDescription, to: from });
      } catch (err) {
    ...
    const onAnswer = async ({ answer }: { answer: RTCSessionDescriptionInit }) => {
      ...
      try {
        ...
        pendingIceRef.current.forEach(c => pcRef.current?.addIceCandidate(new RTCIceCandidate(c)));
        pendingIceRef.current = [];
      } catch (err) {
    ...
    return (
      <div className="min-h-screen w-full flex items-center justify-center bg-gray-900">
        <div className="text-center">
          ...
          <p className="text-gray-400 mb-4">Allow camera access in browser settings and refresh.</p>
          <button onClick={() => router.push('/home')} className="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-lg">
            Go Back
          </button>
        </div>
      </div>
          ...
          <div className="flex items-center gap-2">
            {isCallStarted && (
              <>
                ...
                <span className="text-primary text-sm">REC {fmt(callDuration)}</span>
              </>
            )}
          </div>
        ...
        <section className="flex-1 relative">
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-gray-900">
              <div className="text-center">
                ...
                <p className="text-white text-lg font-medium">
                  {permissionState === 'requesting' && 'Waiting for camera permission...'}
                  {permissionState === 'granted' && waitingForPeer && 'Your camera is ready! Waiting for partner...'}
          ...
          <div className="absolute bottom-10 right-3 lg:w-80 h-45 sm:w-60 sm:h-40 rounded-xl overflow-hidden border border-gray-700 shadow-2xl">
            <video ref={localVideoRef} autoPlay muted playsInline
              className="w-full h-full object-cover scale-x-[-1]" />
            <span className={`absolute bottom-2 right-2 w-3 h-3 rounded-full border-2 border-white
      ...
      {showChat && (
        <aside className="fixed inset-0 z-50 lg:static lg:inset-auto w-full lg:w-96 bg-gray-800 flex flex-col">
          <header className="h-14 px-4 bg-gray-700 flex items-center justify-between">
            ...
            <button onClick={() => setShowChat(false)} className="text-gray-300 hover:text-white text-lg">✕</button>
          </header>
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            <div className="flex gap-3">
              <div className="w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center text-xs text-white">P</div>
              <div>
                <p className="text-gray-400 text-xs">Peer</p>
                <div className="bg-gray-700 text-sm text-white p-3 rounded-lg">Hey, how is the connection?</div>
              </div>
            </div>
          </div>
          <footer className="p-4 bg-gray-700">
            <div className="flex gap-2">
              <input placeholder="Write message..." className="flex-1 bg-gray-600 text-white px-4 py-2 rounded-lg outline-none" />
              <button className="bg-blue-600 hover:bg-blue-700 px-4 rounded-lg text-white">Send</button>
            </div>
          </footer>
        </aside>
      )}
